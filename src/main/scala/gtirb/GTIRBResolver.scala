...
case class GTIRBResolver(mod: Module) {
  ...
  val symbolTabIdxByUuid = AuxDecoder.decodeAux(AuxDecoder.AuxKind.ElfSymbolTabIdxInfo)(mod).map(mapFirst(Uuid.Symbol(_)))
  ...
  val funcNames = decodeAux(AuxKind.FunctionNames)(mod).map {
    case (fun, sym) => Uuid.Function(fun) -> Uuid.Symbol(sym)
  }
  val funcNamesInverse = funcNames.map(_.swap)
  val funcEntries = decodeAux(AuxKind.FunctionEntries)(mod).map {
    case (a, b) => Uuid.Function(a) -> b.map(Uuid.Block(_))
  }
