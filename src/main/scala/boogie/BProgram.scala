...
case class BProgram(declarations: List[BDeclaration]) {
  ...
  def replaceReserved(reserved: Set[String]): BProgram = copy(declarations = declarations.map(d => d.replaceReserved(reserved)))
}
...
case class BProcedure(name: String, in: List[BParam], out: List[BParam], ensures: List[BExpr], requires: List[BExpr], modifies: Set[BVar], body: List[BCmdOrBlock]) extends BDeclaration {
  ...
  override def replaceReserved(reserved: Set[String]): BProcedure = {
    ...
    copy(name = nameUpdate, in = inUpdate, out = outUpdate, ensures = ensuresUpdate, requires = requiresUpdate, modifies = modifiesUpdate, body = bodyUpdate)
  }
}
