...
import util.Boogie.{
  generateBVHeader,
  generateBVToBoolHeader,
  generateLibraryFuncHeader,
  generateSecurityLatticeFuncHeader
}
...
/** The program State
  ...
  *
  * @param controls
  *   the control variables for any given variable
  * @param symbolTable
  *   a mapping from variable id to its location in memory (from the symbol table)
  */
case class State(
  ...
  private def lBodyStr =
    if (L.isEmpty) ";"
      ...
      "{ " + L
        .foldLeft(lattice.top: Sec) { case (prev, (v, p)) =>
  ...
  private def getCompleteRely: List[Pred] = List(
    rely.vars
      .collect { case v: Register => v }
      .foldLeft(rely)((p, v) => p.substExpr(v, exp.`var`.MemLoad(symbolTable(v.name), Some(8)))),
    Forall("i: bv64", "((heap[i] == old(heap[i])) ==> (Gamma_heap[i] == old(Gamma_heap[i])))")
  ) // TODO
...
case object State {
  ...
  def apply(
      ...
      gamma: Map[Register, Security]
  ): State = {
    val controlledBy = lPreds.map { case (v, p) =>
      (v, p.vars)
    }

    // TODO alternatively could use the GOT
    val vars =
      flowGraph.getFunctions.asScala.flatMap(func => func.getInitStmts.asScala.map(init => init.variable)).toSet
...
/** The state of a function
  ...
  *
  * @param labelToBlock
  *   a mapping from a blocks label to the block itself
  * @param rootBlockLabel
  *   the label of the root block
  * @param labelToChildren
  *   a mapping from a blocks label to its children
  */
...
case object FunctionState {
  ...
  def apply(function: FlowGraph.Function): FunctionState = {
    ...
    val labelToChildren = function.getBlocks.asScala
      .map(b =>
        (
          b.label,
          b.lastLine match {
    ...
    new FunctionState(
      blocks,
      function.getInitStmts.asScala.toList,
      function.getHeader,
      function.getBlocks.get(0).label,
      labelToChildren
    )
  }
}
