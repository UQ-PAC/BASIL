...
case class BinOp(op: BinOperator, firstPred: Pred, secondPred: Pred) extends Pred {
  def this(operatorStr: String, firstPred: Pred, secondPred: Pred) =
    this(BinOperator.valueOf(operatorStr), firstPred, secondPred)
  ...
  override def substExpr(v: Var, w: Var) =
    this.copy(firstPred = firstPred.substExpr(v, w), secondPred = secondPred.substExpr(v, w))
}
