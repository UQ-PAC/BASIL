...
case class SecLevelDefn(name: String, parents: List[String]) {
  override def toString =
    s"const unique s_$name: SecurityLevel extends ${parents.map(x => s"s_$x").mkString(", ")} complete;"
}

case object SecLattice {
  val booleanLattice = SecLattice(
    List(SecLevelDefn("FALSE", List()), SecLevelDefn("TRUE", List("FALSE"))),
    SecVar("TRUE"),
    SecVar("TRUE"),
    SecVar("FALSE")
  )
  val TRUE = SecVar("TRUE")
  val FALSE = SecVar("FALSE")
}
