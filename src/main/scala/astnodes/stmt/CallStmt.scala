...
case class CallStmt(
  ...
  override def toString = {
    ...
    val argsStr =
      if (libraryFunction) args.map(arg => arg.name).mkString(", ")
        ...
        CallStmt.callRegisters.map(x => s"$x: bv64").mkString(", ") + ", " + CallStmt.callRegisters
          .map(x => s"Gamma_$x: SecurityLevel")
          .mkString(", ")

    val lhsStr = lhs match {
      case _ if (!libraryFunction) =>
        CallStmt.callRegisters
          .mkString(", ") + ", " + CallStmt.callRegisters.map(x => s"Gamma_$x").mkString(", ") + " := "
  ...
  override def subst(v: Var, w: Var): Stmt = this.copy(args =
    args.map(r =>
      r.subst(v, w) match {
        case x: Register => x
        case _           => ???
