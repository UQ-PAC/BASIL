...
case class GenMapValue(value: Map[BasilValue, BasilValue]) extends BasilValue(None) with MapValue {
  override def toString = s"GenMapValue : $irType"
}

case class Symbol(value: String) extends BasilValue(None)
...
trait NopEffects[T, E] extends Effects[T, E] {
  def loadVar(v: String): State[T, BasilValue, E] = State.pure(Scalar(FalseLiteral))
  def loadMem(v: String, addrs: List[BasilValue]): State[T, List[BasilValue], E] = State.pure(List())
  def evalAddrToProc(addr: Int): State[T, Option[FunPointer], E] = State.pure(None)
  def getNext: State[T, ExecutionContinuation, E] = State.pure(Stopped())
  def setNext(c: ExecutionContinuation): State[T, Unit, E] = State.pure(())

  def call(target: String, beginFrom: ExecutionContinuation, returnTo: ExecutionContinuation): State[T, Unit, E] = State.pure(())
  def callIntrinsic(name: String, args: List[BasilValue]): State[T, Option[BasilValue], E] = State.pure(None)
  def doReturn(): State[T, Unit, E] = State.pure(())

  def storeVar(v: String, scope: Scope, value: BasilValue): State[T, Unit, E] = State.pure(())
  def storeMem(vname: String, update: Map[BasilValue, BasilValue]): State[T, Unit, E] = State.pure(())
}
...
case class MemoryState(
     ...
     */
    stackFrames: Map[StackFrameID, Map[String, BasilValue]] = Map((globalFrame -> Map.empty)),
    activations: List[StackFrameID] = List.empty,
    activationCount: Map[String, Int] = Map.empty.withDefault(_ => 0)
) {
  ...
  def getGlobalVals: Map[String, BitVecLiteral] = {
    stackFrames(globalFrame).collect {
      case (k, Scalar(b: BitVecLiteral)) => k -> b
    }
  }
  ...
  def setVar(v: String, value: BasilValue): MemoryState = {
    val frame = findVarOpt(v).map(_(0)).getOrElse(activations.head)
  ...
  def getVarOpt(name: String): Option[BasilValue] = findVarOpt(name).map(_(1))
  ...
  def doStore(vname: String, values: Map[BasilValue, BasilValue]): Either[InterpreterError, MemoryState] = for {
    _ <- if values.isEmpty then Left(MemoryError("Tried to store size 0")) else Right(())
    v <- findVar(vname)
    (frame, mem) = v
    mapval <- mem match {
        ...
        for {
          m <- values.find((k, v) => k.irType != Some(kt) || v.irType != Some(vt)) match {
            case Some(v) =>
              Left(
                TypeError(
                  s"Invalid addr or value type (${v(0).irType}, ${v(1).irType}) does not match map type $vname : ($kt, $vt)"
                )
              )
            case None => Right(m)
          }
...
object LibcIntrinsic {
  ...
  def intrinsics[S, T <: Effects[S, InterpreterError]]: Map[String, T => State[S, Unit, InterpreterError]] =
...
case class InterpreterState(
  nextCmd: ExecutionContinuation = Stopped(),
  callStack: List[ExecutionContinuation] = List.empty,
...
object NormalInterpreter extends Effects[InterpreterState, InterpreterError] {
  ...
  def loadVar(v: String): State[InterpreterState, BasilValue, InterpreterError] = {
    State.getE((s: InterpreterState) => {
      s.memoryState.getVar(v)
    })
  }

  def evalAddrToProc(addr: Int): State[InterpreterState, Option[FunPointer], InterpreterError] =
    Logger.debug(s"    eff : FIND PROC $addr")
    for {
      res: List[BasilValue] <- State.getE((s: InterpreterState) =>
  ...
  def formatStore(varname: String, update: Map[BasilValue, BasilValue]): String = {
    ...
    val rs = ks.foldLeft(Some((None, List[BitVecLiteral]())): Option[(Option[BigInt], List[BitVecLiteral])]) {
      (acc, v) =>
        v match {
    ...
    rs match {
      case Some(_, l) => 
        ...
        s"$varname[${ks.headOption.map(_(0)).getOrElse("null")}] := $vs"
      case None if ks.length < 8 => s"$varname[${ks.map(_(0)).mkString(",")}] := ${ks.map(_(1)).mkString(",")}"
      case None => s"$varname[${ks.map(_(0)).take(8).mkString(",")}...] := ${ks.map(_(1)).take(8).mkString(", ")}... "
    }

  }

  def loadMem(v: String, addrs: List[BasilValue]): State[InterpreterState, List[BasilValue], InterpreterError] = {
    State.getE((s: InterpreterState) => {
  ...
  def getNext: State[InterpreterState, ExecutionContinuation, InterpreterError] = State.get((s: InterpreterState) => s.nextCmd)

  /** effects * */
  def setNext(c: ExecutionContinuation): State[InterpreterState, Unit, InterpreterError] = State.modify((s: InterpreterState) => {
    s.copy(nextCmd = c)
  })

  def call(target: String, beginFrom: ExecutionContinuation, returnTo: ExecutionContinuation): State[InterpreterState, Unit, InterpreterError] =
  ...
  def doReturn(): State[InterpreterState, Unit, InterpreterError] = {
  ...
  def storeMem(vname: String, update: Map[BasilValue, BasilValue]): State[InterpreterState, Unit, InterpreterError] =
    State.modifyE((s: InterpreterState) => {
