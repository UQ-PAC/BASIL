...
def liftIndirectCall(p: Program) = {
  /**
   * We lift indirect calls to a direct call to an indirectcall trampoline so that DSA correctly indexes indirectcall variables.
   *
   * We perform this transform before parameter form and dsa is applied.
   *
   * We can then define indirect call resolution as the replacement of the call target and alignment of the formal params to the call.
   */
  ...
  class IndirCallReplaceVisitor extends CILVisitor {
    override def vstmt(s: Statement) = s match {
      case i: IndirectCall if s.parent.parent.name != "indirect_call_launchpad" => ChangeTo(List(LocalAssign(callTargetRegister, i.target), DirectCall(indirectCallProc)))
      case _ => SkipChildren()
    }
  }
