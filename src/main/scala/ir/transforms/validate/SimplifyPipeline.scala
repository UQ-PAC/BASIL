...
def simplifyCFG(p: Program) = {
  p.procedures.foreach(transforms.RemoveUnreachableBlocks.apply)
  for (p <- p.procedures) {
...
def validatedSimplifyPipeline(
  ctx: IRContext,
  mode: util.SimplifyMode,
  collectedSnapshots: Option[scala.collection.mutable.ArrayBuffer[IREpoch]] = None
): (TVJob, IRContext) = {
  ...
  var config = mode match {
    case SimplifyMode.ValidatedSimplify(verifyMode, filePrefix, dryRun, effectMode) =>
      TVJob(
        ...
        verify = verifyMode,
        debugDumpAlways = true,
        dryRun = dryRun,
        effects = effectMode,
        logSnapshot = collectedSnapshots
      )
