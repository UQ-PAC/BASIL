...
def convertJumpRenaming(blockName: String => String, varName: CILVisitor, x: Jump): EventuallyJump = x match {
  ...
  case Return(label, out) =>
    EventuallyReturn(
      out.toList.map { case y: (Variable, Expr) =>
        ((y._1.name, visit_expr(varName, y._2)))
      }.toArray,
      label
    )
}
...
def inlineCall(prog: Program, c: DirectCall): Unit = {
  ...
  val internalBlocks = (target.blocks.toSet -- Seq(entry, returnBlock)).map(convertBlockRenaming(renamer, blockRenamer))

  // clone the target procedure and apply the renaming
  val (entryTempBlock, entryResolver) = convertBlockRenaming(renamer, blockRenamer)(entry).makeResolver
  val eventuallyReturnBlock = convertBlockRenaming(renamer, blockRenamer)(returnBlock)
  ...
  val inAssignments = c.actualParams.map { case (formal: LocalVar, actual: Expr) =>
    LocalAssign(visit_rvar(renamer, formal), actual)
  }
