...
def liftLinuxAssertFail(ctx: IRContext) = {
  ...
  case class AssertFail(info: Option[String], filename: Option[String], lineNo: Option[Int], function: Option[String])  extends Replace
  ...
  class ReplaceNondet extends CILVisitor {
    ...
    override def vstmt(s: Statement) = s match {
      ...
      case d: DirectCall if d.target.procName == "__VERIFIER_assert" => {
        ...
        ChangeTo(List(Assert(UnaryExpr(BoolNOT, BinaryExpr(BVEQ, arg, BitVecLiteral(0, 64))), Some("__VERIFIER_assert"))))
      }
      case d: DirectCall if d.target.procName == "__VERIFIER_assume" => {
        val arg = d.actualParams(LocalVar("R0_in", BitVecType(64)))
        ChangeTo(List(Assume(UnaryExpr(BoolNOT, BinaryExpr(BVEQ, arg, BitVecLiteral(0, 64))), Some("__VERIFIER_assume"))))
      }
      case _ => SkipChildren()
    }
  }
