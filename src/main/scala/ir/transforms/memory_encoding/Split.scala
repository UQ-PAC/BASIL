...
case class SplitMemoryEncoding(
  override val repr: MemoryEncodingRepresentation,
  override val simplify: Boolean,
  ctx: IRContext
) extends MemoryEncoding(repr, simplify) {
  override def allocationType: IRType = BitVecType(64)
  override def allocationBType: BType = BitVecBType(64)
  override def allocationLiteral(n: Int): Expr = BitVecLiteral(n, 64)

  private def global_addresses = ctx.symbols.flatMap(s => Range(s.value.intValue, s.value.intValue + s.size)).toSet

  def allocLiveField = BMapVar("alloc_live", MapBType(BitVecBType(64), BitVecBType(2)), Scope.Parameter)
  def allocSizeField = BMapVar("alloc_size", MapBType(BitVecBType(64), BitVecBType(64)), Scope.Parameter)
  def addrIsHeapField = BMapVar("addr_is_heap", MapBType(BitVecBType(64), BoolBType), Scope.Parameter)
  ...
  def addrIsHeapAccess = BMapVar("mem_encoding->#addr_is_heap", MapBType(BitVecBType(64), BoolBType), Scope.Parameter)

  def baseSize: Int = 32
  def offsetSize: Int = 64 - baseSize

  override def memEncodingBDataTypeBody: List[BDataTypeConstructor] = {
    println(allocLiveField)
    List(BDataTypeConstructor("MemEncoding", List(allocLiveField, allocSizeField, addrIsHeapField)))
  }

  override def allocSizeBody: Option[BExpr] =
    Some(MapAccess(allocSizeAccess, pAlloc))

  override def allocBaseBody: Option[BExpr] =
    Some(BinaryBExpr(BVAND, pAlloc, BinaryBExpr(BVSHL, BitVecBLiteral(0xffffffffL, 64), BitVecBLiteral(32, 64))))

  override def addrAllocBody: Option[BExpr] =
    Some(pAddr)

  override def allocLiveBody: Option[BExpr] =
    Some(MapAccess(allocLiveAccess, pAlloc))

  override def addrOffsetBody: Option[BExpr] =
    Some(BinaryBExpr(BVAND, pAddr, BitVecBLiteral(0xffffffffL, 64)))

  override def addrIsHeapBody: Option[BExpr] =
    Some(MapAccess(addrIsHeapAccess, pAddr))

  override def allocSizeUpdateBody: Option[BExpr] =
    Some(BFieldUpdateExpr(pMemEncoding, allocSizeField, MapUpdate(allocSizeAccess, pAlloc, pSize)))

  override def allocLiveUpdateBody: Option[BExpr] =
    Some(BFieldUpdateExpr(pMemEncoding, allocLiveField, MapUpdate(allocLiveAccess, pAlloc, pLive)))

  override def canAllocateBody: Option[BExpr] =
    Some(
      BinaryBExpr(
        BoolAND,
        BinaryBExpr(
          BoolAND,
          // Address is on heap
          addrIsHeapBCall(pAddr, me = pMemEncoding),
          // Address is a base address
          BinaryBExpr(EQ, allocBaseBCall(addrAllocBCall(pAddr, me = pMemEncoding), me = pMemEncoding), pAddr)
        ),
        BinaryBExpr(
          BoolAND,
          // Address is fresh
          BinaryBExpr(
            EQ,
            allocLiveBCall(addrAllocBCall(pAddr, me = pMemEncoding), me = pMemEncoding),
            freshLiteral.toBoogie
          ),
          BinaryBExpr(
            BoolAND,
            // Size <= 2^offset_size - 1
            BinaryBExpr(BVULE, pSize, BitVecBLiteral((BigInt(pow(2, offsetSize).toLong) - 1), 64)),
            // Size > 0
            BinaryBExpr(BVUGT, pSize, BitVecBLiteral(0, 64))
          )
        )
      )
    )

  override def allocateBody: Option[BExpr] =
    Some(
      allocSizeUpdateBCall(
        addrAllocBCall(pAddr, me = pMemEncoding),
        pSize,
        me = allocLiveUpdateBCall(addrAllocBCall(pAddr, me = pMemEncoding), liveLiteral.toBoogie, me = pMemEncoding)
      )
    )

  override def initHeapBody: Option[BExpr] =
    Some(
      BinaryBExpr(
        BoolAND,
        ForAll(
          ...
          BinaryBExpr(
            EQ,
            BinaryBExpr(BVUGT, i.toBoogie, BitVecBLiteral(global_addresses.max, 64)),
            addrIsHeapBCall(i.toBoogie, me = pMemEncoding)
          ),
          triggers = List(List(addrIsHeapBCall(i.toBoogie, me = pMemEncoding)))
        ),
        BinaryBExpr(
          BoolAND,
          ...
          ForAll(
            ...
            BinaryBExpr(
              BoolIMPLIES,
              addrIsHeapBCall(i.toBoogie, me = pMemEncoding),
              BinaryBExpr(EQ, allocLiveBCall(i.toBoogie, me = pMemEncoding), freshLiteral.toBoogie)
            ),
            triggers = List(List(allocLiveBCall(i.toBoogie, me = pMemEncoding)))
          ),
          // ... and non heap addresses are initially dead:
          ForAll(
            List(i.toBoogie),
            BinaryBExpr(
              BoolIMPLIES,
              UnaryBExpr(BoolNOT, addrIsHeapBCall(i.toBoogie, me = pMemEncoding)),
              BinaryBExpr(EQ, allocLiveBCall(i.toBoogie, me = pMemEncoding), deadLiteral.toBoogie)
            ),
            triggers = List(List(allocLiveBCall(i.toBoogie, me = pMemEncoding)))
          )
        )
  ...
  override def validAccessBody: Option[BExpr] =
    Some(
      BinaryBExpr(
        BoolIMPLIES,
        addrIsHeapBCall(pAddr, me = pMemEncoding),
        BinaryBExpr(
          ...
          BinaryBExpr(
            EQ,
            allocLiveBCall(addrAllocBCall(pAddr, me = pMemEncoding), me = pMemEncoding),
            liveLiteral.toBoogie
          ),
          BinaryBExpr(
            BVULE,
            BinaryBExpr(BVADD, addrOffsetBCall(pAddr, me = pMemEncoding), pSize),
            allocSizeBCall(addrAllocBCall(pAddr, me = pMemEncoding), me = pMemEncoding)
          )
        )
      )
    )
...
// case class SplitMemorySpecification() extends MemorySpecification {}

// case class SplitTransform(override val me: MemoryEncoding) extends MemoryEncodingTransform(me) {
//   override val specs = List(SplitCore())
// }
