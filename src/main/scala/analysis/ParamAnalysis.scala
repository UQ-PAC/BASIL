...
class ParamAnalysis(val program: Program) extends Analysis[Any] {
  ...
  private val calledProcs: Set[Procedure] = interLivenessResults.foldLeft(Set(): Set[Procedure]) { (s, map) =>
    map match
  ...
  private def getProcParams(proc: Procedure): Set[Variable] = {
    if completeProcs.contains(proc) then // already know the parameters for this procedure
      ...
      if proc.calls.isEmpty || proc.calls.equals(Set(proc)) then // no call to other functions
        ...
        val calleeParams = proc.calls.intersect(calledProcs).foldLeft(Set(): Set[Variable]) { (s, proc) =>
          s ++ getProcParams(proc)
        }

        val params = interLivenessResults(proc).keys.toSet
          .diff(ignoreRegisters)
          .intersect(intraLivenessResults(proc).union(calleeParams))
        val nonParams = interLivenessResults(proc).keys.toSet
          .diff(ignoreRegisters)
