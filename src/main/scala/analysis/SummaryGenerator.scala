...
class SummaryGenerator(
  ...
  def generateRequires(procedure: Procedure): List[BExpr] = {
    ...
    val (_, mustGammaResults) = worklistSolver(MustGammaDomain(globals, constProp)).solveProc(procedure, false, initialState)
    val (before, after) = worklistSolver(ReachabilityConditions()).solveProc(procedure, false, TrueBLiteral)
