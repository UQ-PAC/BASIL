...
trait ValueSetAnalysis(prog: Program,
  ...
  /** Transfer function for state lattice elements.
    */
  def localTransfer(n: CFGPosition, s: Map[Variable | MemoryRegion, Set[Value]]): Map[Variable | MemoryRegion, Set[Value]] = n match {
    case entry: Procedure =>
      mmm.pushContext(entry.name)
      s
    case cmd: Command =>
      if cmd.parent.statements.lastElem.isDefined && cmd.parent.statements.lastElem.equals(n) && cmd.parent.isProcReturn then {
        mmm.popContext()
        s
      } else {
        eval(cmd, s, n)
...
class ValueSetAnalysisSolver(
    ...
    mmm: MemoryModelMap,
    constantProp: Map[CFGPosition, Map[Variable, FlatElement[BitVecLiteral]]],
) extends ValueSetAnalysis(prog, globals, externalFunctions, globalOffsets, subroutines, mmm, constantProp)
