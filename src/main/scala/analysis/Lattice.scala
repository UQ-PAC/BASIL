...
//  }
  ...
  extension (r: DataRegion | StackRegion)
    ...
    def end(mmm: MemoryModelMap): BigInt = r match {
      case d: DataRegion => mmm.getEnd(d)
      case s: StackRegion => mmm.getEnd(s)
    }
  ...
  def dereference(s: BigInt, vs: ValueSet[String], mmm: MemoryModelMap): (Set[MemoryRegion], Set[MemoryRegion]) = {
    var fullyAccessedLocations = Set[MemoryRegion]()
    var partiallyAccessedLocations = Set[MemoryRegion]()
    vs match {
      case VSBottom => VSBottom
      ...
      case VS(m) =>
        for (elem <- m) {
          if (elem._2 != lattice.bottom) { // region SI defined
            elem._2 match {
                case SI(stride, lower, upper, w) =>
                  // TODO: Global memory size can be retrieved from the symbol table and are of size s
                  // Map addresses to exact memory locations
                  val regionsWithSize = mmm.getRegionsWithSize(s, elem._1)
                  fullyAccessedLocations = fullyAccessedLocations ++ regionsWithSize.filter(region => (region.isInstanceOf[DataRegion | StackRegion] && region.asInstanceOf[DataRegion | StackRegion].start >= lower && region.asInstanceOf[DataRegion | StackRegion].start <= upper) || region.isInstanceOf[HeapRegion]) // TODO: THIS IS NOT THE RIGHT WAY TO CHECK IF ADDRESS IS IN SI

                  // Identify partially accessed locations (if any)
                  val notOfSize = mmm.getRegionsWithSize(s, elem._1, true)
                  // start or end must be in SI
                  partiallyAccessedLocations = partiallyAccessedLocations ++ regionsWithSize.filter(region => region.asInstanceOf[DataRegion | StackRegion].start >= lower && region.asInstanceOf[DataRegion | StackRegion].start <= upper || ((region.asInstanceOf[DataRegion | StackRegion].end(mmm) >= lower) && region.asInstanceOf[DataRegion | StackRegion].end(mmm) <= upper))
                case _ => ???
            }
          }
        }
    ...
    (fullyAccessedLocations, partiallyAccessedLocations)
  }
}
