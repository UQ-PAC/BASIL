...
trait AnalysisPoint {
  ...
  /** Defines equality on two analysis points. Not the same as compare == 0; compare defines an ordering relation where
    * two elements might be on the same level of the lattice, whereas equals is simply asking if the two analyses have
  ...
  /** Fancy method that uses the transfer and compare methods to guarantee that we maintain monotonicity.
    *
    * This shouldn't need to be overridden - write all the analysis-specific stuff in transfer(), compare(), union(),
    * intersection(), createLowest, and toString()!
    */
  ...
  /** Another fancy, please-don't-override method that casts "other" to an instance of "this". Please use this in your
    * transfer, union, intersection, compare, etc. functions though.
    *
    * You might think it would be easier to use scala's subclass comparison thingy where you have
    *
    * AnalysisPoint[A <: AnalysisPoint[A]] { def transfer(stmt: Stmt): A; } ExampleAnalysis(foo) extends
    * AnalysisPoint[ExampleAnalysis]
    *
    * but this causes errors elsewhere in the worklist function where we need to operate on sets of analyses and
