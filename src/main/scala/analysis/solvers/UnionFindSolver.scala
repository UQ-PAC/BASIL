...
class UnionFindSolver[A] {
  ...
  /** This map holds the graph structure for the union-find algorithm. Each term has a "parent". Two terms are
  ...
  /** Returns the canonical element of the equivalence class of the term `t`. The term is added as a new equivalence
    * class if it has not been encountered before. Uses path compression.
    */
  ...
  /** Returns all the unifications of the solution.
    ...
    */
  def unifications(): Map[Term[A], Iterable[Term[A]]] =
