...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val q = BASILConfig(
      ...
      staticAnalysis = if conf.analyse.value then Some(StaticAnalysisConfig(conf.dumpIL, conf.analysisResults, conf.analysisResultsDot, 
        conf.threadSplit.value, conf.summariseProcedures.value, !conf.noIrreducibleLoops.value)) else None,
