...
object BoogieTranslator {
  ...
  def translate(state: State): State =
    inferConstantTypes(
      addVarDeclarations(optimiseSkips(addAssignAtRootBlock(addOutParam(addInParams(createLabels(state))))))
    )
  ...
  private def updateAllBlocks(state: State, fn: Block => Block): State = updateAllFunctions(
    state,
    f =>
      f.copy(labelToBlock = f.labelToBlock.map { case (pc, block) =>
        (pc, fn(block))
      })
  )
  ...
  private def createLabels(state: State): State = {
    val usedLabels = state.functions.flatMap(f =>
      f.labelToBlock.flatMap { case (_, block) =>
        block.lines.collect {
  ...
  /** If a skip is not jumped to, we should remove it. Depends on createLabels TODO: logic doesnt match docstring
    */
  ...
  private def addInParams(state: State): State = updateAllBlocks(
    state,
    block =>
      block.lines(block.lines.size - 1) match {
        case call: CallStmt if (call.libraryFunction) =>
          block.copy(lines =
            block.lines.updated(block.lines.size - 1, call.copy(args = CallStmt.libraryFunctions(call.funcName).args))
          )
        case call: CallStmt =>
          // TODO
          block.copy(lines =
  ...
  private def addOutParam(state: State): State = updateAllLines(
    state,
    stmt =>
      stmt match {
        case call: CallStmt if (call.libraryFunction) =>
        ...
        case call: CallStmt =>
          state.functionFromCall(call).header.getOutParam match {
            case Some(x) =>
  ...
  /** We want to replace mem expressions which represent parameters, like mem[SP + 1], with the human-readable names of
    * those facts.parameters. We do this by first removing the initialising store fact "mem[SP + 1] := X0", then
    * replacing all instances of "mem[SP + 1]" with the variable name. Depends on:
    ...
    updateAllFunctions(
      ...
      function => {
        ...
        val rootBlock = function.rootBlock.copy(lines = function.rootBlock.lines.filter {
          case store: MemAssign =>
            (store.rhs, store.lhs) match {
              case (rhs: Register, lhs: MemLoad) =>
                !paramsWithAliases.exists(param => param.getAlias == lhs && param.getRegister == rhs)
        ...
        for (param <- paramsWithAliases) {
          function.labelToBlock.foreach { case (pc, b) =>
            b.lines.foreach(line => line.subst(param.getAlias, param.getName))
          }
        }
      ...
      b.lines.foreach {
        case RegisterAssign(_, lhs, _) =>
          if (
            ...
            && function.header.getInParams
              .filter((inParam) => inParam.getName.name == lhs.name)
              .isEmpty // TODO check if this is needed (otherwise change to short circuting operation)
  ...
  private def resolveOutParams(state: State): State = {
    ...
    for (function <- state.functions) {
      ...
      if (outParamOp.isDefined) {
        ...
        function.labelToBlock.foreach { case (pc, b) =>
          b.lines.foreach(line => line.subst(outParam.getRegister, outParam.getName))
        }
      }
    }
  ...
  private def addAssignAtRootBlock(state: State) = updateAllFunctions(
    state,
    func =>
      func.copy(
        labelToBlock = func.labelToBlock.updated(
          func.rootBlockLabel,
          func.rootBlock.copy(
            lines = CallStmt.callRegisters
              .map(x => RegisterAssign("NONE", Register(x, 64), Register(s"${x}_in", 64)))
              .toList ++ func.rootBlock.lines
          )
        )
      )
  )

  private def addAssignBeforeReturn(state: State): State = state.copy(functions =
    state.functions.map(f =>
      f.copy(labelToBlock = f.header.getOutParam match {
        case Some(outParam) =>
          f.labelToBlock.map { case (l, b) =>
            (
              l,
              b.copy(lines = b.lines.flatMap {
                case c: ExitSub =>
                  List(
                    RegisterAssign(
                      "generatedline",
                      outParam.getName,
                      Extract(outParam.getName.size.get - 1, 0, outParam.getRegister)
                    ),
                    c
                  )
                case x => List(x)
      ...
      (binOp1.firstExp.size, binOp1.secondExp.size) match {
        ...
        case (a: Some[Int], b: Some[Int]) if (a != b) =>
          throw new AssumptionViolationException(
            s"Both sides of binop ($binOp) should have the same size (${binOp1.firstExp}: ${a}, ${binOp1.secondExp}: ${b})"
          )
