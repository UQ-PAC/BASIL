...
case class SpecGlobal(name: String, override val size: Int, arraySize: Option[Int], address: BigInt) extends SpecGlobalOrAccess {
...
case class ArrayAccess(global: SpecGlobal, index: Int) extends SpecGlobalOrAccess {
  ...
  override def specGlobals: Set[SpecGlobalOrAccess] = Set(this)
  override def resolveSpec: BMemoryLoad = BMemoryLoad(BMapVar("mem", MapBType(BitVecBType(64), BitVecBType(8)), Scope.Global), toAddrVar, Endian.LittleEndian, global.size)
  override def resolveOld: BMemoryLoad = resolveSpec
  override def resolveInsideOld: BExpr = toOldVar
  override def removeOld: BMemoryLoad = resolveSpec
