...
class BitVectorAnalysisTests extends AnyFunSuite {
  test("bvadd1") {
    val result = bvadd(BAPLiteral(5, 8), BAPLiteral(13, 8))
    assert(result == BAPLiteral(18, 8))
  }
  test("bvadd2") {
    val result = bvadd(BAPLiteral(5, 4), BAPLiteral(13, 4))
    assert(result == BAPLiteral(2, 4))
  }
  test("bvadd3") {
    val result = bvadd(BAPLiteral(BigInt("4722366482869645213696"), 128),
      BAPLiteral(BigInt("38685626227668133590597632"), 128))
    assert(result == BAPLiteral(BigInt("38690348594151003235811328"), 128))
  }
  test("bvadd4") {
    val result = bvadd(BAPLiteral(BigInt("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFE", 16), 128), BAPLiteral(4, 128))
    assert(result == BAPLiteral(2, 128))
  }

  test("concat1") {
    val result = concat(BAPLiteral(BigInt("AAAABBBBCCCCDDDD", 16), 64), BAPLiteral(BigInt("EEEEFFFF", 16), 32))
