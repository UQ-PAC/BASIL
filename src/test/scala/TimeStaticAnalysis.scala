...
class TimeStaticAnalysis extends AnyFunSuite {
  ...
  def examples() = {
          ...
          RunUtils
            .staticAnalysis(
              StaticAnalysisConfig(
                memoryRegions = MemoryRegionsMode.DSA
              ),
              ctx
            )
            .timer
    ...
    val times = result.flatMap(x =>
      x._3.map((checkpoint: (String, String, Long)) => {
        val comp = x._2
        (x._1, comp.statements, comp.blocks, comp.procedures, checkpoint._1, checkpoint._3)
      })
    )
  ...
  test("Getexamples") {
    ...
    val logscale = "" // \nset logscale x 2\nset logscale y 2
    var plotfile =
      s"set terminal 'svg' enhanced background rgb 'white'; set output '${outputPathPrefix}/analysisres.svg' ; set xlabel \"statement count\" ; set ylabel \"analysis time (ms)\"$logscale"
