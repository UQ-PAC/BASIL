...
//    assert(dsg.pointTo.size == 12) // 12
    ...
    val framePointer = dsg.find(dsg.adjust(dsg.find(dsg.stackMapping(0).cells(0)).getPointee))
    val R29formal = dsg.find(dsg.adjust(dsg.formals(R29)))
    assert(framePointer.equals(R29formal))
    val stack8 = dsg.find(dsg.stackMapping(8).cells(0)) //  R31 + 8
    assert(dsg.find(dsg.adjust(stack8.getPointee)).equals(dsg.find(dsg.adjust(dsg.formals(R30)))))
    ...
    assert(dsg.find(dsg.adjust(stack32.getPointee)).equals(stack24))
    assert(stack24.node.get.collapsed)
    assert(dsg.find(dsg.adjust(stack24.getPointee)).equals(stack24))

//    assert((stack40.getPointee).equals(((dsg.globalMapping(AddressRange(69600, 69600))._1.cells(0).getPointee)._1.getPointee)))
